...
- **2、组件data定义函数与对象的区别**
  ...
  - 在我们定义好一个组建的时候，vue最终都会通过Vue.extend()构成组件实例
   ...
   - **我们首先可以根据这个属性来防止已经被响应式观察的数据反复被观测，其次响应式数据可以使用__ob__来获取 Observer 实例的相关方法，这对数组很关键**
     ...
     // src/observer/index.js
      ...
      export function observe(data) {
	     ...
	     // 如果data对象已经有__ob__属性了，说明已经被观测过了，之间返回之前的Observer实例就可以了，不需要再新建实例了
	  ...
	  ![vue源码1](D:\knowledge-notes\前端知识\vue\images\vue源码1.png)
