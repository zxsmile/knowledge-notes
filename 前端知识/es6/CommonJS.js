/* 1. 基本概念
   ...
   2. CommonJS的特点
      ...
      - 模块可以多次加载，但只会在第一次加载时运行一次，然后运行结果就会被缓存了，所有缓存的模块保存在require.cache之中,
      以后再加载，就直接读取缓存结果。要想让模块再次运行，必须清除缓存,可以像下面这样写：
   ...
   4. exports对象
    ...
    5. require命令
     ...
     - require加载规则：
          ...
          - 如果参数字符串不以./或/或../开头，说明要加载的不是一个文件，而是一个默认提供的核心模块。
            ...
            在寻找NPM模块包时，会从当前目录出发，向上搜索各级当中的node_modules文件夹当中的文件，但若有两个同名文件，
            则遵循就近原则
                 ...
                 - 之前在使用require的方式来加载一个文件时，如果该文件在当前目录当中，则参数字符串当中必须以./开头，
                 不能直接写文件名,若在参数字符串当中直接写文件名，则代表载入的是一个模块包，模块包必须放在一个特定名字的文件夹当中，
                 即node_modules
          ...
          - 使用require来加载文件时可以省略扩展名。比如require('./module')，则对应的加载顺序为：
              ...
              - 若参数字符串为一个目录（文件夹）的路径，则自动先查找该文件夹下的package.json文件，然后再加载该文件当中main字段
              所指定的入口文件。
    ...
    8.模块的循环加载
         ...
         - CommonJS模块的重要特性是加载时执行，即脚本代码在require的时候，就会全部执行，一旦出现某个模块被循环加载，
         就只输出已执行的部分，还未执行的部分不会输出
             ...
             - 下面一个例子：
                    ...
                    系统会去a.js模块对应对象的exports属性取值，可是因为a.js还没有执行完，从exports属性只能取回已经执行的部分，
                    而不是最后的值
