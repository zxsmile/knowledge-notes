# `Axios`是如何判断其运行环境是`Node.js`还是浏览器的，说一下你的理解?
...
如果需要明确指定运行环境，可以使用`axios.defaults.adapter`属性来设置适配器(`adapter`)，以便在需要时手动选择使用`XMLHttpRequest`或`Node.js`内置的`http` 模块。例如，在`Node.js`环境中可以这样设置适配器:
