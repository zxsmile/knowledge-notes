...
无界用起来感觉和`iframe`差不多，`js`沙箱直接使用`iframe`，`dom`和样式使用`Web Components`，巧妙的避开了`iframe`的缺点 感觉无界就是个缝合怪，这很`Tencent`
...
2. 能否手动（或借助 `node` ）将子项目的文件全部拷贝到主项目服务器上，`node` 监             听子项目文件有更新，就自动拷贝过来，并且按 `js/css/img` 文件夹合并
...
- 同时或先后挂载到同一个页面；
- 使用不同版本的第三方库（如 `jQuery`、`Vue`）；
- 修改全局对象（如 `window.xxx = ...`）；
- 注册全局事件（如 `addEventListener`）。

如果没有隔离机制，会导致：

- **全局变量污染**：`window.xxx = ...` 被覆盖；
- **全局事件监听器冲突**：`addEventListener` 未移除导致内存泄漏；
- **定时器泄漏**：`setTimeout` 在组件卸载后仍在执行；
- **原型链污染**：修改 `Array.prototype` 影响其他代码；
- **安全风险**：恶意脚本访问敏感数据。
- 样式/脚本互相干扰。

因此，**`qiankun` 的沙箱机制旨在实现：**

1. **`JS` 隔离**：子应用对 `window` 的修改在卸载后能还原；/8
2. **副作用清理**：如定时器、事件监听器能被正确回收；
3. **多实例支持**：多个子应用可同时运行而不冲突（仅在某些模式下）。

## `qiankun` 沙箱的核心实现原理

`qiankun` 提供两种沙箱模式：

### 1. **快照沙箱（`Snapshot Sandbox`）** —— 适用于单实例、非严格模式（`IE` 兼容）

> 用于不支持 `Proxy` 的环境（如 `IE11`），或配置 `sandbox: { loose: true }` 时。

#### 原理：

- 在子应用 **激活前**，保存当前 `window` 对象的一个浅拷贝（快照）；
- 子应用运行期间，所有对 `window` 的修改都直接作用于真实 `window`；
- 子应用 **卸载时**，将 `window` 恢复为激活前的状态（通过对比快照）；
- 下次激活时，再把子应用的修改“重新应用”回去。

#### 缺点：

- 不支持多实例（多个子应用不能同时激活）；
- 无法拦截属性访问（只能记录修改，不能动态响应）；
- 性能较差（需遍历 `window` 对象做 `diff`）。
...
Object.keys(window).forEach(key => {
  if (window[key] !== snapshot[key]) {
    delete window[key]; // 或恢复原值
  }
});
```

### 2. **代理沙箱（`Legacy Sandbox / Proxy-based Sandbox`）** —— 默认模式（现代浏览器）

> 基于 `ES6 Proxy` 实现，是 `qiankun` 推荐的沙箱方式。

#### 原理：

- 为每个子应用创建一个 **独立的 `proxy` 对象**，代理对 `window` 的读写；
- 所有子应用代码中的 `window.xxx` 实际操作的是这个 `proxy`；
- `proxy` 内部维护一个 **私有属性池（`fakeWindow`）**，记录该子应用新增/修改的属性；
- 对于未修改的属性，`proxy` 会透传到真实的 `window` 上（`get` 拦截）；
- 子应用卸载时，只需丢弃 `fakeWindow`，无需操作真实 `window`。

#### 关键特性：

- ✅ 支持多实例（每个子应用有自己的 `proxy`）；
- ✅ 动态拦截 `get/set`，更精准；
- ✅ 避免污染全局 `window`；
- ❌ 不支持 `IE`（因依赖 `Proxy`）。
...
function createProxySandbox() {
  ...
  const proxy = new Proxy(window, {
    ...
    get(target, prop) {
      ...
      // 若是函数，bind 回真实 window（避免 this 指向 proxy）
...
> 💡 注意：`qiankun` 还处理了 `this` 指向、`with` 语句、`eval` 等边界情况，实际实现更复杂。

## 沙箱的副作用处理（关键！）

即使使用 Proxy 沙箱，以下**全局副作用仍会污染宿主环境**，需额外劫持：

| 副作用类型                                        | 劫持方案                                              |
| ------------------------------------------------- | ----------------------------------------------------- |
| `window.addEventListener` / `removeEventListener` | 重写方法，记录监听器，在沙箱销毁时自动移除            |
| `setTimeout` / `setInterval`                      | 记录 timer ID，销毁时 `clearTimeout`                  |
| 动态插入 `` / ``                                  | 重写 `document.createElement`，将元素挂载到沙箱容器内 |
| `console.log` 等                                  | 可选重定向到自定义日志系统                            |
| `location.href` 修改                              | 拦截并转为路由跳转（微前端中需统一管理）              |

### 常见副作用类型及 `qiankun` 的解决方案

#### 1. 全局事件监听器（`addEventListener`）

##### 问题：

子应用注册了 `window.addEventListener('resize', handler)`，卸载后未移除，导致内存泄漏或重复触发。

##### `qiankun` 解决方案：**劫持 `addEventListener` / `removeEventListener`**

- 在沙箱激活时，重写 `window.addEventListener`；
- 所有监听器被记录到沙箱私有的 `eventListeners` 数组中；
- 卸载时自动调用 `removeEventListener` 清理。

```
// 简化版实现
function patchEventListeners(sandbox) {
  ...
  window.removeEventListener = function (type, listener, options) {
    // 从记录中移除
    sandbox.eventListeners = sandbox.eventListeners.filter(
...
// 卸载时
function clearEventListeners(sandbox) {
  sandbox.eventListeners.forEach(({ type, listener, options }) => {
    window.removeEventListener(type, listener, options);
  });
}
```

> ✅ `qiankun` 在 `src/sandbox/patchers/eventListener.ts` 中实现了此逻辑。
...
子应用启动了 `setInterval(() => {...}, 1000)`，卸载后仍在运行。

##### `qiankun` 解决方案：**劫持定时器 `API`**

- 重写 `setTimeout` / `setInterval`；
- 记录返回的 `timer ID`；
- 卸载时批量清除。

```
function patchTimers(sandbox) {
  const timers = [];
...
function clearTimers(sandbox) {
  sandbox.timers.forEach(id => {
    clearTimeout(id);
    clearInterval(id);
  });
  sandbox.timers = [];
...
> ⚠️ 注意：`requestAnimationFrame` 也需类似处理（`qiankun` 已支持）。



#### 3.动态插入的 `<script>`和 `<style>`标签

##### 问题本质

子应用在运行时可能通过以下方式动态操作 `DOM`：

```
// 动态插入样式
const style = document.createElement('style');
style.textContent = `.my-btn { color: red; }`;
...
- **样式残留**：卸载子应用后，`` 仍存在于 ``，影响其他应用；
- **脚本持续执行**：外部 `SDK`（如统计、地图）注册全局变量或定时器，无法随子应用销毁；
- **内存泄漏**：`DOM` 节点未移除，引用无法释放。

> ❗ 即使 `JS` 沙箱隔离了 `window` 变量，这些 **`DOM` 操作直接作用于真实文档**，沙箱无法拦截！

##### `qiankun` 的解决方案

`qiankun` 通过 **“资源与容器绑定” + “生命周期自动清理”** 解决此问题。

##### （1）`HTML Entry` 加载阶段预处理

当使用 `entry: '//localhost:8081/index.html'` 时，`qiankun` 底层依赖 `import-html-entry`：

- 下载子应用 `HTML`；
- **提取所有 `` 和 `/`**；
- 将它们的执行/插入**限定在子应用挂载容器内**（而非 `document.head` 或 `document.body`）。
...
```
// 在 mount 阶段
container.appendChild( extractedStyleElement ); // 插入到 #container 内部
execScripts(container, scripts); // 脚本执行时，动态创建的元素也挂到 container
```

##### （2）运行时动态插入的标签：自动重定向

`qiankun` **劫持了部分 `DOM AP`I**（通过 `patchers`），确保动态创建的资源也被约束：

- 重写 `HTMLHeadElement.prototype.appendChild` 等方法；
- 当检测到插入的是 `` 或 ``，自动将其移动到子应用容器内；
- 所有`<script>`的执行上下文也被限制（配合 `JS` 沙箱）。

> ✅ 卸载时，只需清空整个 `container.innerHTML = ''`，所有动态资源自动移除。

##### （3）关键实现位置

- `src/sandbox/patchers/dynamicAppend.ts`：处理动态 append 的 patcher；
- `import-html-entry`：负责初始资源提取和执行。

------

##### ⚠️ 例外与注意事项

| 场景                                                         | 是否被 `qiankun` 捕获 | 说明                                   |
| ------------------------------------------------------------ | --------------------- | -------------------------------------- |
| `document.createElement('style')` + `document.head.appendChild` | ✅（默认开启）         | `qiankun` 会重定向到子应用容器         |
| 直接操作 `document.body.style`                               | ❌                     | 属于 `body` 样式污染，需规范约束       |
| 使用 `innerHTML` 插入 ``                                     | ⚠️ 部分支持            | `qiankun`会尝试提取并执行，但行为复杂  |
| 第三方库内部动态插入（如百度地图）                           | ⚠️ 可能遗漏            | 若库直接操作 `document.head`，可能逃逸 |

> 💡 **建议**：子应用应避免直接操作 `document.head` / `document.body`，所有 DOM 操作限定在自身容器内。
...
> ❗ **`qiankun` 默认不处理 `console`**！因为：
>
> - `console` 不是“状态污染”，不影响功能；
> - 日志属于开发调试行为，应由构建工具（如 terser）在生产环境移除。

##### 🛠️  如何实现 `console` 隔离？

虽然 `qiankun` 不内置，但可**自行实现劫持**：

##### 方案一：在子应用 `mount` 时包装 `console`
...
export async function mount(props) {
  ...
  // 包装
  console.log = (...args) => originalConsole.log(`[${appName}]`, ...args);
  console.error = (...args) => originalConsole.error(`[${appName}]`, ...args);
}
...
console.log = function (...args) {
  // 通过当前激活的子应用名标记（需维护状态）
  const activeApp = getCurrentActiveAppName(); // 自行实现
  if (activeApp) {
    originalLog(`[${activeApp}]`, ...args);
  } else {
    originalLog(...args);
  }
};
...
> ⚠️ 注意：此方案会影响主应用自身日志，需谨慎。

###### 方案三：构建时移除（推荐生产环境使用）

- 使用 `Webpack +terser-webpack-plugin`
      ...
      new TerserPlugin({
        terserOptions: {
          compress: {
            ...
            pure_funcs: ['console.debug'] // 或仅移除特定方法
          }
        }
      })
    ]
...
`qiankun`（乾坤）作为主流微前端框架，对 `CSS` 隔离 的处理采取了 “渐进式 + 可选增强” 策略：默认不强制隔离，但提供实验性能力（`experimentalStyleIsolation`）来缓解样式冲突。下面从原理、实现、局限性和最佳实践四个维度详解。

### 为什么 `CSS` 隔离在微前端中困难？

- 浏览器 **没有原生的 `CSS` 作用域机制**（`Shadow DOM` 除外，但兼容性和生态支持有限）；
- 子应用通常由不同团队开发，可能使用相同类名（如 `.btn`、`.header`）；
- 第三方 `UI` 库（`Ant Design`、`Element Plu`s）大量使用全局类名和 `:root` 变量；
- 动态插入的 `<style>` 标签默认作用于整个文档。

> 🎯 `qiankun` 的目标：**在不破坏子应用原有构建流程的前提下，尽可能减少样式污染**。

### `qiankun` 的 `CSS` 隔离方案

#### ✅ 1. 默认行为：无隔离（透传）

- 子应用的 `CSS`（无论是 `<link>` 还是 `<style>`）**直接插入到主文档的 `<head>`**；
- 样式规则全局生效；
- **优点**：完全兼容现有子应用，无需改造；
- **缺点**：极易发生样式冲突。

> 🔸 这是 `qiankun` 的默认行为，也是其“渐进式微前端”理念的体现——先跑起来，再优化。
...
start({
  sandbox: {
    experimentalStyleIsolation: true // 开启 CSS 沙箱
  }
});
```
...
`qiankun` 在加载子应用 `CSS` 时，动态重写所有选择器，加上一个唯一的属性选择器前缀：

```
/* 子应用原始 CSS */
.header { color: red; }
.ant-btn { font-size: 14px; }

/* qiankun 重写后 */
[data-qiankun="app1"] .header { color: red; }
[data-qiankun="app1"] .ant-btn { font-size: 14px; }
```
...
- 核心逻辑在 `src/sandbox/patchers/css.ts`
- 使用正则表达式解析并重写 `CSS` 规则（非 `AST`，性能较好但有局限）
...
##### ❌ `experimentalStyleIsolation` 的局限性

尽管有用，但它**不是完美的 `CSS` 沙箱**，以下情况无法隔离：

| 问题类型                 | 示例                                              | 是否被隔离              |
| ------------------------ | ------------------------------------------------- | ----------------------- |
| **全局标签选择器**       | `html { font-size: 16px; }` `body { margin: 0; }` | ❌                       |
| **通配符选择器**         | `* { box-sizing: border-box; }`                   | ❌                       |
| **`@keyframes` 动画**    | `@keyframes fade { ... }`                         | ❌（全局注册，可能冲突） |
| **`@font-face` 字体**    | `@font-face { font-family: 'MyFont'; }`           | ❌                       |
| **`:root` `CSS` 变量**   | `:root { --color: red; }`                         | ❌                       |
| **`!important` 覆盖**    | `.btn { color: red !important; }`                 | ⚠️ 可能覆盖主应用样式    |
| **内联样式**             | ``                                                | ❌（不受 CSS 规则影响）  |
| **第三方库动态插入样式** | 某些图表库直接操作 `document.head`                | ⚠️ 可能逃逸              |

> 💡 **根本原因**：`qiankun` 的方案是**选择器重写**，而非真正的作用域隔离。

### 其他辅助隔离手段

#### 动态样式生命周期管理

即使未开启 `experimentalStyleIsolation`，`qiankun` 也会：

- 将子应用的所有 `<style>` 和 `<link>` **记录下来**；
- 在 `unmount` 时**自动移除**这些标签；

✅ 避免样式残留，但**不解决运行时的样式冲突**。

#### 结合 `Shadow DOM`（需手动实现）

#### `qiankun` **不内置 `Shadow DOM`**，但你可以：
...
| 场景                       | 方案                                             |
| -------------------------- | ------------------------------------------------ |
| **快速上线、子应用可控**   | 开启 `experimentalStyleIsolation` + 团队规范约束 |
| **高隔离要求、轻量子应用** | 手动集成 `Shadow DOM`                            |
| **大型项目、多团队协作**   | **规范先行 + 构建时隔离**                        |
...
2. **所有样式必须包裹在子应用根容器内**
   ...
   ```
   /* 推荐：即使不用 qiankun，也这样做 */
   #app1-root .header { ... }
   ```

3. **`UI` 库按需引入 + 自定义前缀**

   - `Ant Design` 支持`prefixCls`：

     ```
     <ConfigProvider prefixCls="app1-antd">
       <App />
     </ConfigProvider>
     ```

   - 生成的类名为 `.app1-antd-btn`，天然隔离。

4. **使用 `CSS Modules` 或 `Scoped CSS`**
...
- 使用 `PostCSS` 插件（如`postcss-prefixwrap`）在打包阶段加前缀：

  ```
  // postcss.config.js
  module.exports = {
    plugins: [
      require('postcss-prefixwrap')('#app1-root')
    ]
  };
  ```

- 优点：100% 覆盖所有 `CSS`，包括 `@keyframes`（需额外配置）

### 验证 `CSS` 隔离是否生效

1. **开启 `experimentalStyleIsolation` 后**：
   - 打开 `DevTools → Elements`；
   - 查看 `<head>` 中的 `<style>` 内容，确认选择器已加 `[data-qiankun="xxx"]` 前缀；
   - 检查子应用容器是否带有 `data-qiankun` 属性。
2. **测试冲突场景**：
   - 主应用定义 `.btn { color: red; }`
   - 子应用定义 `.btn { color: blue; }`
   - 开启隔离后，子应用按钮应为蓝色，主应用按钮仍为红色。

### 总结：`qiankun CSS` 隔离全景

| 方案                             | 隔离强度 | 兼容性     | 是否默认开启 | 适用场景                   |
| -------------------------------- | -------- | ---------- | ------------ | -------------------------- |
| **无隔离（默认）**               | 无       | 全浏览器   | ✅            | 快速验证、内部可信子应用   |
| **`experimentalStyleIsolation`** | 中       | 全浏览器   | ❌            | 多数微前端场景（推荐开启） |
| **Shadow DOM**                   | 强       | 现代浏览器 | ❌            | 高安全/强隔离需求          |
| **构建时加前缀**                 | 高       | 全浏览器   | ❌            | 对稳定性要求高的项目       |

> 🔑 **核心结论**：
>  `qiankun` 的 `CSS` 隔离是“兜底方案”，不是“银弹”。
>  最可靠的隔离 = **团队规范 + 构建工具 + `qiankun` 实验性隔离** 三者结合。
...
![webpack244](.\images\webpack244.png)
