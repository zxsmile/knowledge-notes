# 一、`SPA` 的 `history` 路由模式在 `Nginx` 部署时刷新 404，如何配置解决
...
# 二、如何通过 `Nginx` 配置前端静态资源的 “`hash` 资源永久缓存 + 非 `hash` 资源协商缓存”？
...
# 三、`Nginx` 配置跨域（`CORS`）需设置哪些关键响应头？复杂跨域（带 `cookie`、自定义头）有何注意点
...
## 2、复杂跨域场景：带 `cookie`、自定义头的注意点

当跨域请求需要 **携带 `cookie`** 或 **自定义请求头** 时，配置需更严格，且前后端需协同配合：

#### 带 `cookie` 的跨域（`withCredentials: true`）
...
# 四、`Nginx` 中 `proxy_pass` 路径结尾加 `/` 与不加 `/`，对前端接口代理路径有何差异？举例说明。
...
#  六、`Nginx` 如何配置支持前端大资源的 `Range` 分片请求？核心参数是什么？
...
# 七、`Nginx` 如何按资源类型（如`.js/.png`）分发到不同服务器？配置策略是什么？
...
# 八、`Nginx` 配置 `gzip/brotli` 压缩时需考虑哪些因素？为何不建议对所有前端资源开启压缩？
...
- `brotli` 兼容性
  ...
  ：支持 95% 以上现代浏览器（`Chrome 49+`、`Firefox 44+`、`Edge 15+`），但需注意：
...
### 九、微前端通过 `Nginx` 实现主 / 子应用路由分发，如何配置 `location` 和 `try_files`？需处理哪些资源路径问题？
...
```nginx
nginx 代码解读复制代码# HTTP 服务器（80端口）：仅用于跳转 HTTPS
server {
    listen 80;
...
4. **配置验证**：
    修改配置后需执行 `nginx -t` 检查语法，通过 `nginx -s reload` 生效；可使用 [SSL Labs 工具](https://link.juejin.cn?target=https%3A%2F%2Fwww.ssllabs.com%2Fssltest%2F) 检测配置安全性（目标评分 A+）。
...
```nginx
nginx 代码解读复制代码location ~* \.(js|css|png|jpg|jpeg|webp|mp4)$ {
...
```nginx
nginx 代码解读复制代码# 缓存打开的文件描述符（减少重复打开文件的磁盘 I/O 耗时）
...
```nginx
nginx 代码解读复制代码# 全局限制：单个请求体最大 100MB（根据业务调整，如图片站可设 50MB）
...
```nginx
nginx 代码解读复制代码# 在 nginx.conf 全局配置中
...
1. **静态资源托管到 CDN**：
   ...
   ```nginx
   nginx 代码解读复制代码location ~* \.(js|css|png)$ {
...
```nginx
nginx 代码解读复制代码# nginx.conf 全局配置
...
# 十一、`Nginx` 如何为不同前端资源配置缓存策略？如何强制刷新特定资源？
