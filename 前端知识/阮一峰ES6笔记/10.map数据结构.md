...
为了解决`JavaScript`对象的键名只能是字符串的问题，`ES6` 提供了 **`Map` 数据结构。它类似于对象，也是键值对的集合，但是“键”的范围不限于字符串，各种类型的值（包括对象）都可以当作键。**
...
m.has(o) // false
```
**`Map` 构造函数可以接受一个数组作为参数或者任何具有`Iterator` 接口、且每个成员都是一个双元素的数组的数据结构（详见《`Iterator`》一章）都可以当作`Map` 构造函数的参数。这就是说，`Set`和`Map` 都可以用来生成新的 `Map` 。**
